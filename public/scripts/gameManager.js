...
const gameManager = (() => {
  ...
  const doFrame = (now) => {
    if (window.gameState.gameStartTime === 0)
      window.gameState.gameStartTime = now;
  ...
  const setupSocketHandlers = (socket) => {
    ...
    socket.on("player_disconnected", (playerId) => {
      ...
      const index = window.gameState.players.findIndex(
        (p) => p.id === playerId,
      );
      console.log(
        `Player disconnected: ${player ? player.name : "Unknown"} (${playerId})`,
      );
      if (index !== -1) {
    ...
    window.addEventListener("beforeunload", () => {
      socket.emit("close_connection", {
        playerName: window.gameState.selfPlayerName,
