...
app.post('/register', async (req, res) => {
    ...
    if (!username || !avatar || !name || !password) 
        return res.json({ error: 'Username/avatar/name/password cannot be empty.' });
...
app.post('/signin', async (req, res) => {
    ...
    const userInfo = { username, avatar: users[username].avatar, name: users[username].name };
...
io.on('connection', (socket) => {
    ...
    socket.on('cancel_private_room', (code) => {
        ...
        for (const sid of Object.keys(room.players)) {
            ...
            if (s) {
                s.leave(code);
                if (sid !== socket.id) s.emit('room_error', 'Private room cancelled by owner');
            }
        }
        delete rooms[code];
    });
    ...
    socket.on('ready', (code) => {
        ...
        io.to(code).emit('room_update', room);
        const allReady = Object.values(room.players).length > 0 && Object.values(room.players).every(p => p.ready);
        if (allReady) 
            io.to(code).emit('game_start');
    });
